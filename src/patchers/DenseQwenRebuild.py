...
class Qwen3MLP(nn.Module):
    def __init__(self, config, layer_idx: int):
        ...
        self.gate_proj = nn.Linear(self.hidden_size, compressed_int_size, bias=False)
        self.up_proj = nn.Linear(self.hidden_size, compressed_int_size, bias=False)
        self.down_proj = nn.Linear(compressed_int_size, self.hidden_size, bias=False)
...
class Qwen3Attention(nn.Module):
    ...
    def __init__(self, config: Qwen3Config, layer_idx: int, layer_rotary_mask: torch.Tensor | None):
        super().__init__()
        self.config = config
    ...
    def _masked_rms_norm(self, x: torch.Tensor, norm: Qwen3RMSNorm, is_query: bool) -> torch.Tensor:
        """
        Apply RMSNorm with per-head weight gathering via the rotary mask.
        Each compressed head may keep different original dimensions, so we gather
        the corresponding norm weights for each head independently.

        x: [batch, seq, n_heads, compressed_head_dim]
        norm: Qwen3RMSNorm with weight [original_head_dim]
        """
        input_dtype = x.dtype
        x_float = x.to(torch.float32)
        variance = x_float.pow(2).mean(-1, keepdim=True)
        normed = x_float * torch.rsqrt(variance + norm.variance_epsilon)

        mask = self.layer_rotary_mask  # [n_kv_heads, compressed_head_dim]
        if is_query:
            mask = torch.repeat_interleave(mask, self.num_key_value_groups, dim=0)
        # mask: [n_heads, compressed_head_dim] — indices into original_head_dim

        gathered_weight = norm.weight[mask.to(norm.weight.device)]  # [n_heads, compressed_hd]
        gathered_weight = gathered_weight.unsqueeze(0).unsqueeze(
            1
        )  # [1, 1, n_heads, compressed_hd]

        return (gathered_weight * normed).to(input_dtype)
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        hidden_shape = (*input_shape, -1, self.head_dims)

        raw_q = self.q_proj(hidden_states).view(hidden_shape)
        raw_k = self.k_proj(hidden_states).view(hidden_shape)

        if self.layer_rotary_mask is not None:
            query_states = self._masked_rms_norm(raw_q, self.q_norm, is_query=True).transpose(1, 2)
            key_states = self._masked_rms_norm(raw_k, self.k_norm, is_query=False).transpose(1, 2)
        else:
            query_states = self.q_norm(raw_q).transpose(1, 2)
            key_states = self.k_norm(raw_k).transpose(1, 2)
